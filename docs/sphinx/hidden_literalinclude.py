...
class hidden_literalinclude(nodes.General, nodes.FixedTextElement):
    pass
...
class Hiddenliteralinclude(LiteralInclude):
    ...
    def run(self):
        ...
        code = u'\n'.join(self.content)
        hcb = hidden_literalinclude(code, code)
...
def visit_hcb_html(self, node):
    ...
    Aliteralinclude = self.body[-1]

    fill_header = {'divname': 'hiddenliteralinclude{0}'.format(HCB_COUNTER), 
    ...
    Aliteralinclude = js_showhide + divheader + Aliteralinclude + "</div>"

    # reassign and exit
    self.body[-1] = Aliteralinclude
...
def setup(app):
    app.add_directive('hidden_literalinclude', Hiddenliteralinclude)
    app.add_node(hidden_literalinclude, html=(visit_hcb_html, depart_hcb_html))
